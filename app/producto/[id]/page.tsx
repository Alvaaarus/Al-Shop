...
export default function ProductoPage({ params }: Props) {
  ...
  const caracteristicasLista =
    producto.caracteristicas?.length ? producto.caracteristicas : producto.etiquetas.map((t) => `Detalle: ${t}`);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-10">
      <nav className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-dark-500 mb-6 md:mb-8">
        <Link href="/" className="hover:text-smoke transition-colors duration-300">
          Inicio
        </Link>
        <span>/</span>
        <Link href="/#drops" className="hover:text-smoke transition-colors duration-300">
          Limited Drop
        </Link>
        <span>/</span>
      ...
      {/* Layout inspirado en Nike: miniaturas | visor | ficha */}
      <div
        className={cn(
      ...
      >
        {/* Miniaturas verticales (orden: 1ª img, 2ª img, video) */}
        {mediaSlots.length > 1 && (
          <div className="flex lg:flex-col gap-2 order-2 lg:order-1 overflow-x-auto lg:overflow-visible pb-1 lg:pb-0 -mx-1 px-1 lg:mx-0 lg:px-0 snap-x snap-mandatory lg:snap-none">
            {mediaSlots.map((slot, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'relative shrink-0 w-[4.25rem] h-[4.25rem] lg:w-full lg:aspect-square overflow-hidden border bg-dark-800 snap-start transition-all duration-200 touch-manipulation',
                    isActive ? 'border-smoke ring-1 ring-smoke/40' : 'border-dark-600 hover:border-dark-400 active:border-dark-300'
                  )}
                  aria-label={
                    slot.kind === 'video'
                ...
                >
                  {slot.kind === 'imagen' ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-900 text-[9px] uppercase tracking-[0.12em] text-neon-cyan gap-0.5">
        ...
        <div className={cn('order-1 lg:order-2', mediaSlots.length <= 1 && 'xl:col-span-1')}>
          <div className="relative aspect-square w-full max-w-3xl mx-auto bg-[#141414] border border-dark-600">
            {producto.nuevo && (
          ...
          {/* Móvil: pista de que hay más medios si solo hay 1 columna de thumbs arriba */}
          {mediaSlots.length > 1 && (
            <p className="mt-2 text-center text-[10px] text-dark-500 uppercase tracking-[0.2em] lg:hidden">
              Desliza las miniaturas o tócalas para cambiar vista
            </p>
          )}
        </div>

        {/* Ficha producto (estilo Nike, tema AL SHOP) */}
        <div
          className={cn(
        ...
        >
          ...
          {/* Modelo (en lugar de selector de tallas) */}
          <div>
          ...
          <p className="text-sm text-dark-400 leading-relaxed border-l-2 border-dark-600 pl-4">{producto.descripcion}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <div className="flex items-center border border-dark-600 h-12 bg-dark-900/50">
              <button
                ...
                className="w-12 h-full flex items-center justify-center text-xl text-dark-300 hover:bg-dark-800 transition-colors"
              >
                −
              </button>
            ...
            <button
              ...
              className={cn(
                'flex-1 h-12 text-sm uppercase tracking-[0.15em] font-medium transition-all duration-300 rounded-none',
                agregado
          ...
          {producto.stock > 0 && producto.stock <= 8 && (
            <p className="text-[10px] uppercase tracking-wider text-neon-red">Solo quedan {producto.stock} unidades</p>
          )}
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 border border-dark-600 h-12 text-sm text-dark-300 hover:border-neon-cyan hover:text-neon-cyan transition-all duration-300 uppercase tracking-[0.12em]"
          >
      ...
      <div>
        <div className="flex items-end justify-between mb-6">
          <h2 className="font-display text-2xl md:text-3xl uppercase tracking-wider text-white">También te puede gustar</h2>
          <Link
            href="/#drops"
